...
bool8 madenoise;              // true when shooting or screaming
...
#ifdef BSS_SECTION_TILEMAP
BSS_SECTION_TILEMAP
#endif
uint8_t tilemap[MAPSIZE][MAPSIZE]; // wall values only

#ifdef BSS_SECTION_ACTORAT
BSS_SECTION_ACTORAT
#endif
uint8_t actorat[MAPSIZE][MAPSIZE];

#ifdef BSS_SECTION_SPOTVIS
BSS_SECTION_SPOTVIS
#endif
uint32_t spotvis[(MAPAREA) >> 5];
...
bool8 buttonheld[NUMBUTTONS];

bool8 demorecord, demoplayback;
...
{
    if (InputStatus & IN_Flags_KeyAttack) buttonstate[bt_attack] = true;
    if (InputStatus & IN_Flags_KeyStrafe) buttonstate[bt_strafe] = true;
    if (InputStatus & IN_Flags_KeyUse) buttonstate[bt_use] = true;
    if (InputStatus & IN_Flags_KeyStrafeLeft) buttonstate[bt_strafeleft] = true;
    if (InputStatus & IN_Flags_KeyStrafeRight) buttonstate[bt_straferight] = true;
    if (InputStatus & IN_Flags_KeyEsc) buttonstate[bt_esc] = true;
    if (InputStatus & IN_Flags_KeyPause) buttonstate[bt_pause] = true;
    if (InputStatus & IN_Flags_KeyPrevWeapon) buttonstate[bt_prevweapon] = true;
    if (InputStatus & IN_Flags_KeyNextWeapon) buttonstate[bt_nextweapon] = true;
    if (InputStatus & IN_Flags_KeyEsc) buttonstate[bt_esc] = true;
}
...
{
    uint32_t delta = (InputStatus & IN_Flags_KeyRun)?
                RUNMOVE * tics : BASEMOVE * tics;

    if (InputStatus & IN_Flags_KeyUp)
        controly -= delta;
    if (InputStatus & IN_Flags_KeyDown)
        controly += delta;
    if (InputStatus & IN_Flags_KeyLeft)
        controlx -= delta;
    if (InputStatus & IN_Flags_KeyRight)
        controlx += delta;
}
...
{
    RETROPORT_StartDamageFlash (damage);
//    damagecount += damage;
}
...
void UpdatePaletteShifts (void)
{
    ...
    if (red)
    {
        VL_SetPalette (&redshifts_l[(red - 1) * PALSIZE], false);
        palshifted = true;
    }
    else if (white)
    {
        VL_SetPalette (&whiteshifts_l[(white - 1) * PALSIZE], false);
        palshifted = true;
    }
    else if (palshifted)
    {
...
void DoActor (objtype * ob)
{
    ...
    if (!(ob->flags & (FL_NONMARK | FL_NEVERMARK)))
        actorat[ob->tilex][ob->tiley] = ACTORAT_EMPTY;
...
//
    ...
    if (!ob->ticcount)
    {
        ...
        actorat[ob->tilex][ob->tiley] = ACTORAT_OBJLIST_IDX(
                                            OBJLIST_GET_PTR2IDX(ob));
        return;
    }
