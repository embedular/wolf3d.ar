...
void SpawnNewObj (unsigned tilex, unsigned tiley, const statetype *state)
{
    ...
    actorat[tilex][tiley] = ACTORAT_OBJLIST_IDX(OBJLIST_GET_PTR2IDX(newobj));
...
{                                                   \
    ...
    {                                               \
        if (!ACTORAT_IS_OBJLIST_IDX(av))            \
            return false;                           \
        if (ACTORAT_GET_OBJLIST_PTR(av)->flags & FL_SHOOTABLE)  \
            return false;                           \
    }                                               \
}
    ...
    #define DOORCHECK                                   \
            if(DEMOCOND_ORIG)                           \
                doornum = ACTORAT_GET_DOORNUM(av);      \
            else                                        \
            {                                           \
                doornum = ACTORAT_GET_DOORNUM(av);      \
                OpenDoor(doornum);                      \
...
{                                                       \
    ...
    if (av)                                             \
    {                                                   \
        if (ACTORAT_IS_ANY_TILE(av))                    \
            return false;                               \
        if (ACTORAT_IS_DOORNUM(av))                     \
        {                                               \
            DOORCHECK                                   \
        }                                               \
        else if (ACTORAT_IS_OBJLIST_IDX(av)             \
                    && ACTORAT_GET_OBJLIST_PTR(av)->flags & FL_SHOOTABLE) \
            return false;                               \
    }                                                   \
}
...
void DropItem (wl_stat_t itemtype, int tilex, int tiley)
{
    ...
    //
    if (actorat[tilex][tiley] == ACTORAT_EMPTY)
    {
