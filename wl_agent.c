...
static const statetype   s_player = {false,0,0,(statefunc) T_Player,NULL,NULL};
static const statetype   s_attack = {false,0,0,(statefunc) T_Attack,NULL,NULL};
...
{
    RETROPORT_StatusDrawFace (picnum);
}
...
static void LatchNumber (int x, int y, unsigned width, int32_t number)
{
    ...
    while (c<length)
    {
        ...
        RETROPORT_StatusDrawNumber (x, y, str[c]-'0');
        x += 8;
        c++;
    }
}
...
void DrawHealth (void)
{
    ...
    LatchNumber (27, 124, 0, gamestate.health);
}
...
void DrawWeapon (void)
{
    ...
    RETROPORT_StatusDrawWeapon (KNIFEPIC+gamestate.weapon);
}
...
void DrawAmmo (void)
{
    ...
    LatchNumber (213, 124, 2, gamestate.ammo);
}
...
#define PUSHWALLMINDIST PLAYERSIZE
    ...
    for (y=yl;y<=yh;y++)
    {
        for (x=xl;x<=xh;x++)
        {
            ...
            if (av && ACTORAT_IS_OBJLIST_IDX(av))
            {
                check = ACTORAT_GET_OBJLIST_PTR(av);
                if (check != player && (check->flags & FL_SHOOTABLE) )
                {
                    deltax = ob->x - check->x;
