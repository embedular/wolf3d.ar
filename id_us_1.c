...
#define SSTART_STACK_BUFFER_SIZE    512
...
{
    char sstart[SSTART_STACK_BUFFER_SIZE];
    assert (strlen(sorg) < sizeof(sstart));
    strncpy (sstart, sorg, sizeof(sstart));
...
///////////////////////////////////////////////////////////////////////////
void
US_CenterWindow(uint16_t w,uint16_t h)
{
...
{
    if(randomize)
        rndindex = (RETROPORT_GetTicks() >> 4) & 0xff;
    else
        rndindex = 0;
}
