...
#define TIMEY   8
    ...
    VL_FadeOut (0, 255, C6(0), C6(17), C6(17), 5);
#endif
...
PG13 (void)
{
    VW_FadeOut ();
#if 0
    VWB_Bar (0, 0, RETROPORT_SCREEN_WIDTH, RETROPORT_SCREEN_HEIGHT, 0x82);     // background

    VWB_DrawPic (160, 74, PG13PIC);
#endif
    RETROPORT_FillFromStorage (RETROPORT_CACHED_ELEMENT_PG13);
...
//    CacheLump (LEVELEND_LUMP_START, LEVELEND_LUMP_END);
    ...
    VWB_Bar (0, 0, RETROPORT_SCREEN_WIDTH, RETROPORT_SCREEN_HEIGHT, VIEWCOLOR);
...
//
...
#ifndef JAPAN
#ifdef SPANISH
        sprintf (tempstr, "piso %u completo", gamestate.mapon + 1);
        Write (0, 1, tempstr);
#else
        sprintf (tempstr, "floor completed");
        Write (0, 1, tempstr);
        // Write (14, 2, "floor\ncompleted");
#endif
    ...
    #define TREASURE_Y  128
        ...
        Write (BONUS_X - strlen(STR_BONUS) * 16, 22, STR_BONUS);
        Write (BONUS_X - 16, BONUS_Y, "0");

        Write (107, TIME_Y, STR_TIME);
        Write (107, PAR_Y, STR_PAR);

#ifdef SPANISH
        Write (46, KILL_Y, STR_RATKILL);
        Write (46, SECRET_Y, STR_RATSECRET);
        Write (46, TREASURE_Y, STR_RATTREASURE);
#else
        Write (46, KILL_Y, STR_RATKILL);
        Write (46, SECRET_Y, STR_RATSECRET);
        Write (46, TREASURE_Y, STR_RATTREASURE);
...
#ifdef SPANISH
        Write (PAR_X, PAR_Y, parTimes[gamestate.episode * 10 + mapon].timestr);
#else
        Write (PAR_X, PAR_Y, parTimes[gamestate.episode * 10 + mapon].timestr);
        // Write (94, PAR_Y, parTimes[gamestate.episode * 10 + mapon].timestr);
#endif
...
/*
        ...
        i = 26 * 8;
#endif
        VWB_DrawPic (i, 59, L_NUM0PIC + (min / 10));
        i += 2 * 8;
        VWB_DrawPic (i, 59, L_NUM0PIC + (min % 10));
        i += 2 * 8;
        Write (i / 8, 59, ":");
        i += 1 * 8;
        VWB_DrawPic (i, 59, L_NUM0PIC + (sec / 10));
        i += 2 * 8;
        VWB_DrawPic (i, 59, L_NUM0PIC + (sec % 10));
*/
        sprintf (tempstr, "%u%u:%u%u", min / 10, min % 10, sec / 10, sec % 10);
        Write (TIME_X, TIME_Y, tempstr);
        ...
        if (bonus)
        {
            for (i = 0; i <= timeleft; i++)
            {
                ...
                x = BONUS_X - (int) strlen(tempstr) * 16;
                Write (x, BONUS_Y, tempstr);
        ...
        {
            itoa (i, tempstr, 10);
            x = RATIO_X - (int) strlen(tempstr) * 16;
            Write (x, KILL_Y, tempstr);
            if (!(i % 10))
            {
                SD_PlaySound (ENDBONUS1SND);
            }
...
#endif
#else
        Write (0, 4, "secret floor\n completed!");
#endif

        Write (0, 6, "15000 bonus!");
...
PreloadUpdate (unsigned current, unsigned total)
{
    ...
    if (w)
    {
        VWB_Bar (WindowX + 5, WindowY + WindowH - 3, w, 2, 224); // 0x37);   //SECONDCOLOR);
        VWB_Bar (WindowX + 5, WindowY + WindowH - 3, w - 1, 1, SECONDCOLOR); // 0x32);
    }
    VW_UpdateScreen ();
...
{
    LatchDrawPic ((screenWidth-224)/16, (screenHeight-48)/2, GETPSYCHEDPIC);
    WindowX = (screenWidth - 224)/2;
    WindowY = (screenHeight - 48)/2;
    WindowW = 28 * 8;
    WindowH = 48;
...
//    CA_CacheGrChunk (C_NAMEPIC);
    ...
    VWB_DrawPic (2 * 8, 0, HIGHSCORESPIC);

#ifndef APOGEE_1_0
    VWB_DrawPic (2 * 8, 68, C_NAMEPIC);
    VWB_DrawPic (16 * 8, 68, C_LEVELPIC);
    VWB_DrawPic (23 * 8, 68, C_SCOREPIC);
#else
