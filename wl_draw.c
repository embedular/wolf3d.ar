...
#define DOORWALL        98 // (PMSpriteStart-8)
...
#ifdef BSS_SECTION_WALLHEIGHT
BSS_SECTION_WALLHEIGHT
#endif
uint16_t wallheight[RETROPORT_SCREEN_WIDTH];
static uint16_t min_wallheight;

//
// math tables
//
// (IN ROM) short pixelangle[RETROPORT_SCREEN_WIDTH];
// (IN ROM) int32_t finetangent[FINEANGLES/4];
// (IN ROM) fixed sintable[ANGLES+ANGLES/4];
...
// word horizwall[MAXWALLTILES],vertwall[MAXWALLTILES]; (IN ROM)
...
static const uint8_t rgb332_shade_1down_lookup[256] = {

        
};


static const uint8_t *postsource;
int postx;
int postwidth;
// sgermino:    just to know if postsource will be used in a horizontal or
//              vertical tile in ScalePost()
static uint8_t  postsource_v;
...
void ScalePost()
{
    ...
    yoffs = (viewheight / 2 - ywcount) * RETROPORT_SCREEN_WIDTH;
    if(yoffs < 0) yoffs = 0;
    yoffs += postx;
...
#else
    #ifndef TEXTURE_BAKED_SHADING
            if (postsource_v)
            {
                col = rgb332_shade_1down_lookup[postsource[yw]];
            }
            else
            {
                col = postsource[yw];
    ...
    #endif
#endif
    yendoffs = yendoffs * RETROPORT_SCREEN_WIDTH + postx;
    while(yoffs <= yendoffs)
    {
        ...
        yendoffs -= RETROPORT_SCREEN_WIDTH;
    }
}
...
uint8_t vgaCeiling[]=
{
    ...
    72,72,72,73,75,72,73,64,73,75,
    73,73,73,73,73,64,68,73,73,2,

    73,1,64,68,68,1,64,108,73,68,
    13,73,64,64,68,137,73,73,73,64,
    73,73,73,73,68,68,13,68,68,68
/*
 // gamepal[x]
...
#else
    for(y = 0; y < viewheight / 2; y++, ptr += RETROPORT_SCREEN_WIDTH)
        memset(ptr, ceiling, viewwidth);
    for(; y < viewheight; y++, ptr += RETROPORT_SCREEN_WIDTH)
        memset(ptr, 109, viewwidth);
#endif
}
...
int CalcRotate (objtype *ob)
{
    ...
    if (ob->state->rotate == ro_pain)   // 2 rotation pain frame
            ...
            while(lpix<rpix)
            {
                if(wallheight[lpix]<=(int)height)
                {
                    ...
                    while((endy = READWORD(&line)) != 0)
                    {
                        ...
                        else vmem=vbuf+screndy*RETROPORT_SCREEN_WIDTH+lpix;
                        for(;j<endy;j++)
                        {
                            scrstarty=screndy;
                            ycnt+=pixheight;
...
//
    for (statptr = &statobjlist[0] ; statptr !=laststatobj ; statptr++)
    {
        ...
        if (!SPOTVIS_CHECKXY(statptr->tilex, statptr->tiley))
...
//
    for (obj = player->next;obj;obj=obj->next)
    {
        ...
        //
        if ((    SPOTVIS_CHECK(spotloc-1) && !*(tilespot-1) )
            || ( SPOTVIS_CHECK(spotloc+1) && !*(tilespot+1) )
            || ( SPOTVIS_CHECK(spotloc-65) && !*(tilespot-65) )
            || ( SPOTVIS_CHECK(spotloc-64) && !*(tilespot-64) )
            || ( SPOTVIS_CHECK(spotloc-63) && !*(tilespot-63) )
            || ( SPOTVIS_CHECK(spotloc+65) && !*(tilespot+65) )
            || ( SPOTVIS_CHECK(spotloc+64) && !*(tilespot+64) )
            || ( SPOTVIS_CHECK(spotloc+63) && !*(tilespot+63) ) )
        {
...
void DrawPlayerWeapon (void)
{
    ...
    {
        if (gamestate.weapon < -1 || gamestate.weapon > wp_chaingun)
        {
            LOG_Warn (NOBJ, "BUG?");
            LOG_Items (1, "gamestate.weapon", (int32_t)gamestate.weapon);
            return;
        }
...
//
    ...
    uint32_t curtime = RETROPORT_GetTicks();
    tics = (curtime * 7) / 100 - lasttimecount;
    if(!tics)
    {
        // wait until end of current tic
        RETROPORT_Delay(((lasttimecount + 1) * 100) / 7 - curtime);
        tics = 1;
    }
...
//
    ...
    SPOTVIS_SETXY(player->tilex, player->tiley);
